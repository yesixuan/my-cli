...
module.exports = class Creator extends EventEmitter {
  ...
  async create (cliOptions = {}, preset = null) {
    ...
    // inject core service
    preset.plugins['@vicli/cli-service'] = Object.assign({
      projectName: name
    }, preset)
    if (cliOptions.bare) {
      preset.plugins['@vicli/cli-service'].bare = true
    }
    ...
    deps.forEach(dep => {
      ...
      pkg.devDependencies[dep] = (
        ...
        ((/^@vicli/.test(dep)) ? `^${latestMinor}` : `latest`)
      )
    })
    // write package.json
  ...
  async resolvePlugins (rawPlugins) {
    ...
    rawPlugins = sortObject(rawPlugins, ['@vicli/cli-service'], true)
    const plugins = []
